...
@interface WGBVideoCapture () <AVCaptureVideoDataOutputSampleBufferDelegate>
@property (nonatomic, strong, readwrite) WGBVideoCaptureConfig *config;
...
@implementation WGBVideoCapture
...
- (void)_updateMirror {
    ...
    if ([connection isVideoMirroringSupported]) {
        if ((self.config.mirrorType & WGBVideoCaptureMirrorFront) && self.config.position == AVCaptureDevicePositionFront) {
            connection.videoMirrored = YES;
        } else if ((self.config.mirrorType & WGBVideoCaptureMirrorBack) && self.config.position == AVCaptureDevicePositionBack) {
            connection.videoMirrored = YES;
        } else {
