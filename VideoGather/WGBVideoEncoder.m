...
@interface WGBVideoEncoder ()
@property (nonatomic, assign) VTCompressionSessionRef compressionSession;
@property (nonatomic, strong, readwrite) WGBVideoEncoderConfig *config; // 视频编码配置参数。
...
@implementation WGBVideoEncoder
...
- (void)encodePixelBuffer:(CVPixelBufferRef)pixelBuffer ptsTime:(CMTime)timeStamp {
    ...
    dispatch_async(self.encoderQueue, ^{
        ...
        if (!weakSelf.compressionSession) {
            ...
            if (weakSelf.retrySessionCount >= KFEncoderRetrySessionMaxCount && weakSelf.errorCallBack) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    weakSelf.errorCallBack([NSError errorWithDomain:NSStringFromClass([WGBVideoEncoder class]) code:setupStatus userInfo:nil]);
                });
            }
            return;
        }
        ...
        if (weakSelf.encodeFrameFailedCount >= KFEncoderEncodeFrameFailedMaxCount && weakSelf.errorCallBack) {
            dispatch_async(dispatch_get_main_queue(), ^{
                weakSelf.errorCallBack([NSError errorWithDomain:NSStringFromClass([WGBVideoEncoder class]) code:encodeStatus userInfo:nil]);
            });
        }
    });
}
...
static void encoderOutputCallback(void * CM_NULLABLE outputCallbackRefCon,
    ...
    // 向外层回调编码数据。
    WGBVideoEncoder *videoEncoder = (__bridge WGBVideoEncoder *) outputCallbackRefCon;
