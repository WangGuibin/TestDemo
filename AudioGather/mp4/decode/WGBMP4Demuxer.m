...
#define WGBMP4DemuxerBadFileError 2000
#define WGBMP4DemuxerAddVideoOutputError 2001
#define WGBMP4DemuxerAddAudioOutputError 2002
#define WGBMP4DemuxerQueueMaxCount 3

@interface WGBMP4Demuxer () {
    CMSimpleQueueRef _audioQueue;
    CMSimpleQueueRef _videoQueue;
}
@property (nonatomic, strong, readwrite) WGBDemuxerConfig* config;
...
@property (nonatomic, assign, readwrite) WGBMP4DemuxerStatus demuxerStatus; // 解封装器状态。
...
- (instancetype)initWithConfig:(WGBDemuxerConfig *)config {
    ...
    if (self) {
        ...
        _demuxerStatus = WGBMP4DemuxerStatusUnknown;
        _demuxerQueue = dispatch_queue_create("com.KeyFrameKit.demuxerQueue", DISPATCH_QUEUE_SERIAL);
        CMSimpleQueueCreate(kCFAllocatorDefault, WGBMP4DemuxerQueueMaxCount, &_audioQueue);
        CMSimpleQueueCreate(kCFAllocatorDefault, WGBMP4DemuxerQueueMaxCount, &_videoQueue);
    }
    return self;
}
...
- (void)dealloc {
    // 清理状态机。
    if (self.demuxerStatus == WGBMP4DemuxerStatusRunning) {
        self.demuxerStatus = WGBMP4DemuxerStatusCancelled;
    }
...
- (void)startReading:(void (^)(BOOL success, NSError *error))completeHandler {
    ...
    dispatch_async(_demuxerQueue, ^{
        ...
        if (!weakSelf.demuxReader) {
            ...
            weakSelf.demuxerStatus = error ? WGBMP4DemuxerStatusFailed : WGBMP4DemuxerStatusRunning;
...
- (void)cancelReading {
    ...
    dispatch_async(_demuxerQueue, ^{
        ...
        weakSelf.demuxerStatus = WGBMP4DemuxerStatusCancelled;
...
- (BOOL)hasAudioSampleBuffer {
    // 是否还有音频数据。
    if (self.hasAudioTrack && self.demuxerStatus == WGBMP4DemuxerStatusRunning && !self.audioEOF) {
...
- (CMSampleBufferRef)copyNextAudioSampleBuffer CF_RETURNS_RETAINED {
    ...
    while (!sampleBuffer && self.demuxerStatus == WGBMP4DemuxerStatusRunning && !self.audioEOF) {
        // 先从缓冲队列取数据。
        dispatch_semaphore_wait(_audioQueueSemaphore, DISPATCH_TIME_FOREVER);
        if (CMSimpleQueueGetCount(_audioQueue) > 0) {
        ...
        // 缓冲队列没有数据，就同步加载一下试试。
        if (!sampleBuffer && self.demuxerStatus == WGBMP4DemuxerStatusRunning) {
...
- (BOOL)hasVideoSampleBuffer {
    // 是否还有视频数据。
    if (self.hasVideoTrack && self.demuxerStatus == WGBMP4DemuxerStatusRunning && !self.videoEOF) {
...
- (CMSampleBufferRef)copyNextVideoSampleBuffer CF_RETURNS_RETAINED {
    // 拷贝下一份视频采样。
    CMSampleBufferRef sampleBuffer = NULL;
    while (!sampleBuffer && self.demuxerStatus == WGBMP4DemuxerStatusRunning && !self.videoEOF) {
        // 先从缓冲队列取数据。
        dispatch_semaphore_wait(_videoQueueSemaphore, DISPATCH_TIME_FOREVER);
        if (CMSimpleQueueGetCount(_videoQueue) > 0) {
...
- (void)_setupDemuxReader:(NSError**)error {
    ...
    // 3、处理待解封装的资源中的视频。
    if (self.config.demuxerType & WGBMediaVideo) {
        // 取出视频轨道。
        ...
        if (_hasVideoTrack) {
            ...
            if ([_demuxReader canAddOutput:_readerVideoOutput]) {
                ...
                *error = _demuxReader.error ? _demuxReader.error : [NSError errorWithDomain:NSStringFromClass([self class]) code:WGBMP4DemuxerAddVideoOutputError userInfo:nil];
                return;
            }
        }
    }
    ...
    // 4、处理待解封装的资源中的音频。
    if (self.config.demuxerType & WGBMediaAudio) {
        // 取出音频轨道。
        ...
        if (_hasAudioTrack) {
            ...
            if ([_demuxReader canAddOutput:_readerAudioOutput]) {
                ...
                *error = _demuxReader.error ? _demuxReader.error : [NSError errorWithDomain:NSStringFromClass([self class]) code:WGBMP4DemuxerAddAudioOutputError userInfo:nil];
                return;
            }
        }
    }
    ...
    if (!_hasVideoTrack && !_hasAudioTrack) {
        *error = [NSError errorWithDomain:NSStringFromClass([self class]) code:WGBMP4DemuxerBadFileError userInfo:nil];
        return;
    }
...
- (void)_loadNextSampleBuffer {
    ...
    if (self.demuxReader.status == AVAssetWriterStatusCompleted) {
        self.demuxerStatus = WGBMP4DemuxerStatusCompleted;
        return;
        ...
        if (self.demuxReader.status == AVAssetWriterStatusFailed) {
            // 如果状态依然是失败，就上报错误。
            self.demuxerStatus = WGBMP4DemuxerStatusFailed;
    ...
    BOOL audioNeedLoad = (self.config.demuxerType & WGBMediaAudio) && !self.audioEOF;
    BOOL videoNeedLoad = (self.config.demuxerType & WGBMediaVideo) && !self.videoEOF;
    while (self.demuxReader && self.demuxReader.status == AVAssetReaderStatusReading && (audioNeedLoad || videoNeedLoad)) {
        ...
        if (audioNeedLoad) {
            ...
            dispatch_semaphore_signal(_audioQueueSemaphore);
            if (audioCount < WGBMP4DemuxerQueueMaxCount) {
                // 从音频输出源读取音频数据。
        ...
        if (videoNeedLoad) {
            ...
            dispatch_semaphore_signal(_videoQueueSemaphore);
            if (videoCount < WGBMP4DemuxerQueueMaxCount) {
                // 从视频输出源读取视频数据。
