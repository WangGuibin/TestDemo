...
#define WGBMP4MuxerAddOutputError 1000
#define WGBMP4MuxerMaxQueueCount 10000

// 封装器的状态机。
typedef NS_ENUM(NSInteger, WGBMP4MuxerStatus) {
    WGBMP4MuxerStatusUnknown = 0,
    WGBMP4MuxerStatusRunning = 1,
    WGBMP4MuxerStatusFailed = 2,
    WGBMP4MuxerStatusCompleted = 3,
    WGBMP4MuxerStatusCancelled = 4,
};


@interface WGBMP4Muxer (){
    CMSimpleQueueRef _audioQueue; // 音频数据队列。
    CMSimpleQueueRef _videoQueue; // 视频数据队列。
}

@property (nonatomic, strong, readwrite) WGBMuxerConfig *config;
...
@property (nonatomic, assign) WGBMP4MuxerStatus muxerStatus;

@end

@implementation WGBMP4Muxer
...
- (instancetype)initWithConfig:(WGBMuxerConfig *)config {
    ...
    if (self) {
        ...
        CMSimpleQueueCreate(kCFAllocatorDefault, WGBMP4MuxerMaxQueueCount, &_audioQueue);
        CMSimpleQueueCreate(kCFAllocatorDefault, WGBMP4MuxerMaxQueueCount, &_videoQueue);
    }
...
- (void)startWriting {
    ...
    dispatch_async(self.muxerQueue, ^{
        ...
        [weakSelf _reset]; // 清理。
        weakSelf.muxerStatus = WGBMP4MuxerStatusRunning; // 标记状态。
...
- (void)cancelWriting {
    ...
    dispatch_async(self.muxerQueue, ^{
        ...
        weakSelf.muxerStatus = WGBMP4MuxerStatusCancelled; // 标记状态。
        dispatch_semaphore_signal(weakSelf.semaphore);
    });
}

- (void)appendSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    if (!sampleBuffer || !CMSampleBufferGetDataBuffer(sampleBuffer) || self.muxerStatus != WGBMP4MuxerStatusRunning) {
        return;
    }
    ...
    dispatch_async(self.muxerQueue, ^{
        ...
        if (!weakSelf.muxWriter) {
            ...
            if (!success) {
                weakSelf.muxerStatus = WGBMP4MuxerStatusFailed;
                CFRelease(sampleBuffer);
                dispatch_semaphore_signal(weakSelf.semaphore);
...
- (void)stopWriting:(void (^)(BOOL success, NSError *error))completeHandler {
    ...
    dispatch_async(self.muxerQueue, ^{
        ...
        [weakSelf _stopWriting:^(BOOL success, NSError *error) {
            weakSelf.muxerStatus = success ? WGBMP4MuxerStatusCompleted : WGBMP4MuxerStatusFailed;
            dispatch_semaphore_signal(weakSelf.semaphore);
            if (completeHandler) {
...
- (BOOL)_setupMuxWriter:(NSError **)error {
    if (!self.config.outputURL) {
        *error = [NSError errorWithDomain:NSStringFromClass([WGBMP4Muxer class]) code:40003 userInfo:nil];
        return NO;
    }
    ...
    if ((self.config.muxerType & WGBMediaVideo) && !_writerVideoInput) {
        ...
        if ([self.muxWriter canAddInput:self.writerVideoInput]) {
            ...
            *error = self.muxWriter.error ? self.muxWriter.error : [NSError errorWithDomain:NSStringFromClass([WGBMP4Muxer class]) code:WGBMP4MuxerAddOutputError userInfo:nil];
            return NO;
        }
    }
    
    // 4、当封装内容包含音频时，创建 Muxer 的音频输入。
    if ((self.config.muxerType & WGBMediaAudio) && !_writerAudioInput) {
...
- (void)_avInterLeavedSample {
    ...
    if ((self.config.muxerType & WGBMediaAudio) && (self.config.muxerType & WGBMediaVideo)) { // 同时封装音频和视频。
    ...
    } else if (self.config.muxerType & WGBMediaAudio) { // 只封装音频。
        [self _appendAudioSample];
    } else if (self.config.muxerType & WGBMediaVideo) { // 只封装视频。
        [self _appendVideoSample];
    }
}
...
- (BOOL)_checkFormatDescriptionLoadSuccess {
    // 检查数据是否正常。
    if (!_muxWriter) {
        if ((self.config.muxerType & WGBMediaAudio) && (self.config.muxerType & WGBMediaVideo)) {
            return CMSimpleQueueGetCount(_videoQueue) > 0 && CMSimpleQueueGetCount(_audioQueue) > 0;
        } else if (self.config.muxerType & WGBMediaAudio) {
            return CMSimpleQueueGetCount(_audioQueue) > 0;
        } else if (self.config.muxerType & WGBMediaVideo) {
