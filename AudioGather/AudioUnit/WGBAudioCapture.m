...
@property (nonatomic, strong, readwrite) WGBAudioConfig *config;
...
@implementation WGBAudioCapture
...
- (void)startRunning {
    ...
    dispatch_async(_captureQueue, ^{
        ...
        if (startStatus != noErr) {
            // 捕捉并回调开始采集时的错误。
            [weakSelf callBackError:[NSError errorWithDomain:NSStringFromClass([WGBAudioCapture class]) code:startStatus userInfo:nil]];
        }
    });
}
...
- (void)stopRunning {
    ...
    dispatch_async(_captureQueue, ^{
        if (weakSelf.audioCaptureInstance) {
            ...
            if (stopStatus != noErr) {
                // 捕捉并回调停止采集时的错误。
                [weakSelf callBackError:[NSError errorWithDomain:NSStringFromClass([WGBAudioCapture class]) code:stopStatus userInfo:nil]];
            }
        }
    });
}
...
static OSStatus audioBufferCallBack(void *inRefCon,
    ...
    @autoreleasepool {
        WGBAudioCapture *capture = (__bridge WGBAudioCapture *) inRefCon;
        if (!capture) {
            return -1;
        }
        ...
        if (status == noErr) {
            ...
            CMSampleBufferRef sampleBuffer = [WGBAudioCapture sampleBufferFromAudioBufferList:buffers inTimeStamp:inTimeStamp inNumberFrames:inNumberFrames description:capture.audioFormat];
            // 回调数据。
